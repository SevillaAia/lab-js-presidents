...
function getNames(presidentsArr) {
  return presidentsArr.map((onePres) =>
     onePres.name);
  }
...
function  countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce((total, president) => {
    if(president.leftOffice !== null && president.tookOffice !== null){
      const yearServed = president.leftOffice - president.tookOffice;
      return total + yearServed;
    } 
    return total;
  }, 0)
}
...
function countRepublicanPresidents(presidentsArr) {
   return presidentsArr.reduce((accu, president) =>{
    return president.party === "Republican" ? accu + 1 : accu;
  }, 0);
}
